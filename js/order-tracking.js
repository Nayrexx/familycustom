...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function buildTimeline(currentStatus, orderDate, statusHistory) {
        ...
        statuses.forEach((status, index) => {
            ...
            if (index < currentIndex) {
                ...
                timeline[status] = {
                    completed: true,
                    date: historyEntry ? formatDateTime(historyEntry.date) : getEstimatedDate(orderDate, index)
                };
                ...
                timeline[status] = {
                    completed: false,
                    active: true,
                    date: historyEntry ? formatDateTime(historyEntry.date) : 'En cours'
                };
            } else {
                // Étapes futures
                timeline[status] = {
                    completed: false,
                    date: getEstimatedDate(orderDate, index)
                };
            }
        });
    ...
    // Estimation des dates selon l'étape
    function getEstimatedDate(orderDate, stepIndex) {
        const estimates = [0, 0, 1, 4, 7]; // Jours après commande
        const estimatedDate = new Date(orderDate);
        estimatedDate.setDate(estimatedDate.getDate() + estimates[stepIndex]);
        
        if (stepIndex >= 3) {
            return 'Estimation : ' + formatDate(estimatedDate);
        }
